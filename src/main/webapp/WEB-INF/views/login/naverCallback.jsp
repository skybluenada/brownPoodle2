...
<html>
<head>
<title>네이버로그인 : 갈색푸들</title>
</head>
<body>
	<%
	System.out.println("NAVER CallBack >>> : ");
	String clientId = "auFKWzgp26tOU_2jRrOj";//애플리케이션 클라이언트 아이디값";
	String clientSecret = "Yiap4L4YfJ";//애플리케이션 클라이언트 시크릿값";
	String code = request.getParameter("code");
	String state = request.getParameter("state");
	String redirectURI = URLEncoder.encode("http://localhost:8088/callback.bp", "UTF-8");
	String apiURL;
	...
	try {
		if (session.getAttribute("state") != null && session.getAttribute("state").equals(state)) {
			...
			if (responseCode == 200) {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/naverlogin.bp");
