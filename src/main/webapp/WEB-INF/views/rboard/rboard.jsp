...
<%
	...
	if (nick == null) System.out.println("null 이다.");
	
	System.out.println("rboard jsp 페이지의 닉네임: " + nick);
%>
	...
	<script type="text/javascript">
		$(document).ready(function () {
						...
						if (data != null && data != '') {
							$(data).each(function () {
								...
								str += '<form id="' + this.rb_num + '" name="' + this.rb_num + '">' 
									...
									+ '<tr><td class="rbcontent_name"><span class="rb_username">' + this.rb_name + '</span><br><span class="rbcontent_name2">' + this.rb_insertdate //이름 날짜
									+ '</span></td>' 
									+ '<td class="rbcontent_width" colspan="2"><span class="rb_content_' + this.rb_num + '">' + this.rb_content  //댓글 내용
									+ '</span><input type="hidden" name="rb_content" id="rb_content"></td>';
									
									/*
								if (nick == this.rb_name) {
									
									str +=  '<tr><td class="rboard_edit" colspan="2">'
									...
									*/
									str += '<td class="boardadd" style="float: right;">'
									+ '<div class="add">'
				    				...
				    				+ '<a class="dropdown-item" tabindex="0" id="r_update_btn" name="r_update_btn">수정</a>'
				    				+ '<a class="dropdown-item" tabindex="0" id="r_delete_btn" name="r_delete_btn">삭제</a>'
				  					+ '</div>' 
									+ '</div>'
									+ '</td>';
			...
			*/
			...
			// 댓글 입력 글자 수 제한
			// // 댓글 250자 제한 함수 --------------------------------------------
			// function cut_250(obj){
			// 	var t = $(obj).val();
			// 	var l = t.length;
			// 	while(getTextLength(t) > 250){
			// 		l--;
			// 		t = t.substring(0, l);
			// 	}
			// 	$(obj).val(t);
	...
	<form id="rboardWrite" name="rboardWrite">
		<table class="rboard">
			<tr>
						<td colspan="4" style= "background-color: #f9f7f7; border-radius: 8px;">
					<span id="rb_name" name="rb_name" class="rbname">비회원</span>
<%
				if (nick == null || nick.equals("null")) { // 
%>

					<input type="text" id="rb_content" name="rb_content" class="rbcontent" placeholder="로그인이 필요한 서비스입니다." readonly>
<%
				} else {
%>
					<input type="text" id="rb_content" name="rb_content" class="rbcontent">
<%
				}
%>
				</td>	
<%
				if (nick != null && nick != "null") { // 로그인 상태라면 댓글 쓰기 가능
%>

				<td class="r_button_td" style="background-color: #f9f7f7;
   			 	border-radius: 8px;">
				<input type="button" id="r_insert_btn" name="r_insert_btn" class="r_button" value="입력하기" >
				</td>		
<%
				}
%>
